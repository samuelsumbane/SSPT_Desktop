...
import com.samuelsumbane.ssptdesktop.modules.clientsRoutes
import com.samuelsumbane.infrastructure.routes.ownersRoutes
import com.samuelsumbane.infrastructure.routes.suppliersRoutes
import com.samuelsumbane.repository.HttpResponseText
import com.samuelsumbane.repository.UsersRepository
import com.samuelsumbane.ssptdesktop.AUDIENCE
...
//            updateUserPassword
        ...
        if (AppConfig.hasModule("suppliers")) suppliersRoutes()
        if (AppConfig.hasModule("sales")) reportRoutes()
        if (AppConfig.hasModule("products")) productsRoutes()
        if (AppConfig.hasModule("clients")) clientsRoutes()
        if (AppConfig.hasModule("categories")) categoriesRoutes()
        if (AppConfig.hasModule("stock")) stockRoutes()
        if (AppConfig.hasModule("branches")) branchesRoutes()
        if (AppConfig.hasModule("payables")) payableRoutes()
        if (AppConfig.hasModule("receivables")) receivableRoutes()
        if (AppConfig.hasModule("owners")) ownersRoutes()
        if (AppConfig.hasModule("notifications")) notificationsRoutes()
        if (AppConfig.hasModule("logs")) logsRoutes()
//        if (AppConfig.hasModule("")) Routes()
    }
}
